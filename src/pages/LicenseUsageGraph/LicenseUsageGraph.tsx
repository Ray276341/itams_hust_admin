...
import { getUsageByLicenseId } from "../../api/licenseUsage";
import { License, LicenseUsage } from "../../interface/interface";
import {
  LineChart,
  Line,
  XAxis,
...
export default function LicenseUsageGraph() {
  const { licenseId: paramId } = useParams<{ licenseId: string }>();
  const defaultLicenseId = Number(paramId);
  const navigate = useNavigate();

  const [allLicenses, setAllLicenses] = React.useState<License[]>([]);
  const [selectedLicenses, setSelectedLicenses] = React.useState<number[]>([
    defaultLicenseId,
  ]);
  const [usageMap, setUsageMap] = React.useState<
    Record<number, LicenseUsage[]>
  >({});
  ...
  React.useEffect(() => {
    getAllLicenses()
      .then(setAllLicenses)
      .catch(() => console.error("Failed loading licenses"));
  }, []);
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    Promise.all(
      selectedLicenses.map((sid) =>
        getUsageByLicenseId(sid).then((list) => ({ sid, list }))
      )
    )
      .then((results) => {
        const map: Record<number, LicenseUsage[]> = {};
  ...
  const handleLicenseChange = (e: SelectChangeEvent<number[]>) =>
    setSelectedLicenses(e.target.value as number[]);
  ...
  selectedMetrics.forEach((metric) => {
    let maxVal = 0;
    selectedLicenses.forEach((svcId) => {
      ...
      <Paper sx={{ p: 3, backgroundColor: "#FFF" }}>
        ...
        <Box display="flex" gap={2} flexWrap="wrap" mb={3}>
          <FormControl sx={{ minWidth: 200 }}>
            <InputLabel id="svc-label">Licenses</InputLabel>
            <Select
              labelId="svc-label"
              id="svc-select"
              label="Licenses"
        ...
        <Grid container spacing={4}>
          {selectedLicenses.map((svcId) => {
            const svcName =
              allLicenses.find((s) => s.id === svcId)?.name || `#${svcId}`;
            return (
