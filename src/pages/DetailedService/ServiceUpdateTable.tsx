...
import { ServiceUpdate } from "../../interface/interface";
...
interface EnhancedTableProps {
  ...
  onRequestSort: (
    ...
    property: keyof ServiceUpdate
  ) => void;
  onSelectAllClick: (event: React.ChangeEvent<HTMLInputElement>) => void;
  order: Order;
  orderBy: keyof ServiceUpdate;
  rowCount: number;
}
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof ServiceUpdate) => (event: React.MouseEvent<unknown>) =>
...
export default function ServiceUpdateTable(props: { serviceId: number }) {
  const { serviceId } = props;
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = React.useState<keyof ServiceUpdate>("id");
  ...
  const [rows, setRows] = React.useState<ServiceUpdate[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getServiceUpdatesByServiceId(serviceId);
      setRows(data);
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <Box sx={{ width: "100%" }}>
          ...
          <Box sx={{ display: "flex", gap: 1 }}>
            {selected.length === 0 && (
              <Button
                ...
                onClick={() =>
                  navigate(`/services/${serviceId}/updates/create`)
                }
              >
                Update
              </Button>
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  const labelId = `serviceupdate-checkbox-${index}`;
                  return (
                    <TableRow
                      hover
                      role="checkbox"
                    ...
                    >
                      ...
                      <TableCell align="left">
                        <Actions
                          id={row.id}
                          path={`services/${serviceId}/updates`}
                          data={row}
                          notClone
                          notUpdate
                          onClickDelete={() => {
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        <DialogTitle>Delete Service Update?</DialogTitle>
        <DialogContent>
          <DialogContentText>
