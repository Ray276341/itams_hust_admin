...
import {
  getOutgoingLicenseDependencies,
  getIncomingLicenseDependencies,
  deleteLicenseDependency,
} from "../../api/licenseDependency";
import { getLicenseById } from "../../api/license";
import { LicenseDependency } from "../../interface/interface";
import Actions from "../../components/Actions";
import { toast } from "react-toastify";
import LicenseDependencyDiagram from "./LicenseDependencyDiagram";

const headCells = [
  { id: "id", label: "ID", numeric: false },
  { id: "licenseName", label: "License", numeric: false },
...
interface EnhancedTableProps {
  ...
  onRequestSort: (
    e: React.MouseEvent,
    property: keyof LicenseDependency
  ) => void;
  order: Order;
  orderBy: string;
  rowCount: number;
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof LicenseDependency) => (e: React.MouseEvent) => {
...
export default function LicenseDependencyTable({
  licenseId,
}: {
  licenseId: number;
}) {
  const sid = Number(licenseId);
  const navigate = useNavigate();

  const [license, setLicense] = React.useState<{ name: string } | null>(null);
  React.useEffect(() => {
    getLicenseById(sid).then(setLicense);
  }, [sid]);

  const useTableState = (
    fetchFn: (id: number) => Promise<LicenseDependency[]>
  ) => {
    const [rows, setRows] = React.useState<LicenseDependency[]>([]);
    const [order, setOrder] = React.useState<Order>("asc");
    const [orderBy, setOrderBy] = React.useState<keyof LicenseDependency>("id");
    ...
    const handleDelete = async (id: number) => {
      try {
        await deleteLicenseDependency(id);
  ...
  const outgoingState = useTableState(getOutgoingLicenseDependencies);
  const incomingState = useTableState(getIncomingLicenseDependencies);
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          {state.selected.length === 0 ? (
            <Button
              ...
              onClick={() =>
                navigate(
                  `/licenses/${sid}/dependencies/create?direction=${direction}`
                )
              }
            >
              New
            ...
            {state.rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell
                    sx={{
                      width: columnWidthMap["licenseName"],
                      maxWidth: columnWidthMap["licenseName"],
                      whiteSpace: "nowrap",
                      overflow: "hidden",
                      textOverflow: "ellipsis",
                    }}
                  ...
                  >
                    <Actions
                      id={row.id}
                      path={`licenses/${sid}/dependencies`}
                      data={row}
                      notClone
                      notUpdate
                      onClickDelete={() => {
  ...
  return (
    <Box sx={{ display: "flex", gap: 2, p: 2 }}>
      ...
      >
        ...
        {license && (
          <LicenseDependencyDiagram
            outgoing={outgoingState.rawRows.map((r) => ({
              id: `${r.id}`,
              name: r.dependencyName,
              relationshipName: r.relationshipName,
            }))}
            current={{ id: `svc-${licenseId}`, name: license.name }}
