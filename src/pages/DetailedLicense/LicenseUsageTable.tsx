...
import { LicenseUsage, Asset } from "../../interface/interface";
...
const headCells: readonly HeadCell[] = [
  ...
  { id: "asset", disablePadding: false, numeric: false, label: "Assetname" },
  {
    id: "usage_metric",
...
interface EnhancedTableProps {
  ...
  onRequestSort: (
    ...
    property: keyof LicenseUsage
  ) => void;
  onSelectAllClick: (event: React.ChangeEvent<HTMLInputElement>) => void;
  order: Order;
  orderBy: keyof LicenseUsage;
  rowCount: number;
}
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof LicenseUsage) => (event: React.MouseEvent<unknown>) =>
...
export default function LicenseUsageTable(props: { licenseId: number }) {
  const { licenseId } = props;
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = React.useState<keyof LicenseUsage>("id");
  ...
  const [rows, setRows] = React.useState<LicenseUsage[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getUsageByLicenseId(licenseId);
      setRows(data);
    } catch (err) {
      console.error(err);
    }
            ...
            {selected.length === 0 && (
              <>
                <Button
                  ...
                  onClick={() => navigate(`/licenses/${licenseId}/graph`)}
                >
                  Graph
                </Button>
                <Button
                  ...
                  onClick={() =>
                    navigate(`/licenses/${licenseId}/usage/create`)
                  }
                >
                  New
                </Button>
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  const labelId = `licenseusage-checkbox-${index}`;
